...
const MealDetails = () => {
  ...
  useEffect(() => {
    const fetchMeal = async () => {
      try {
        ...
        if (!response.ok) throw new Error(data.error || "Failed to fetch meal");

        setMeal(data);
        setLikeCount(data.likes?.length || 0); // Initialize like count
        setRequestCount(data.requestCount || 0); // Initialize request count
        setReviews(data.reviews || []); // Initialize reviews
        setHasLiked(data.userHasLiked || false); // Initialize liked status
      } catch (error) {
        console.error("Error fetching meal details:", error);
      } finally {
        setLoading(false);
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setLikeCount(result.updatedLikes || likeCount + 1); // Update like count
        setHasLiked(true); // Mark as liked
      } else {
        console.error("Failed to like meal:", result.error);
      }
    } catch (error) {
      console.error("Error liking the meal:", error);
    }
  ...
  const handleMealRequest = async () => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        setRequestCount(result.mealRequest.requestCount || requestCount + 1); // Update request count
        if (result.mealRequest.review) {
          setReviews((prev) => [...prev, result.mealRequest.review]); // Add new review
        }
        setReviewText(""); // Clear review text
      } else {
        console.error("Failed to request meal:", result.error);
      }
    } catch (error) {
      console.error("Error requesting the meal:", error);
    }
  ...
  return (
    <div className="meal-details p-5 bg-gray-100 rounded-lg w-9/12 mx-auto">
      ...
      <p className="text-gray-700 mt-2">Posted: {meal.postTime || "N/A"}</p>
      <p className="text-gray-700 mt-2">Rating: {meal.rating || "N/A"}</p>
